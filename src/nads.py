    ...
    *,
    ...
    for seedset in neighbors:
        ...
        if candidate_spread > best_spread:
            ...
            history.append([best_spread, time.monotonic() - start, calls])
...
def nads(
    ...
    *,
    ...
    verbose: int = 0,
) -> tuple[list[float], list[np.ndarray], list[list[float]]]:
    ...
    spread_history, seed_history, evaluation_history
        Accepted objective values, their binary seed vectors, and every best
        improvement as ``[spread, elapsed_seconds, objective_calls]``.
    """
    ...
    evaluation_history = [[initial_spread, time.monotonic() - start, 1]]
    calls = 1
    xi_t = float(xi)
    ...
    if evaluation_history[-1][0] != spread_history[-1]:
        evaluation_history.append(
            [spread_history[-1], time.monotonic() - start, calls]
        )
    _print_status(
        verbose,
        calls,
